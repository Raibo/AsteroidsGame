...
Ссылка на билд: [Build.rar](https://github.com/Raibo/AsteroidsGame/raw/master/Build.rar)
...
Лазер не проходит через границу игрового поля и не выходит с другой стороны.  
Если бы лазер проходил, то тогда получилось бы так, что у лазера либо должна быть ограниченная длина, либо он заполнит собой все игровое поле.  
Оба эти варианта не показались мне улучшающими игровой опыт.  
...
### Реализации игровых механик, от которых ничего не зависит, не имеют интерфейса
Так сделано потому что в таком интерфейсе нет необходимости.  
Раз нет зависимости, то нет и потребности в публичных методах/свойствах, а значит и не понятно, что должен содержать интерфейс.  
Можно конечно предположить, что он должен содержать, но это уже предсказательное программирование, после которого всегда приходится переделывать.  
Лучше сделать тогда, когда будет нужно.  
